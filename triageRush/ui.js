...
(function buildUiNamespace() {
  ...
  const elementIds = [
    ...
    "reviewPastShiftsButton", "resetPastShiftsButton",
    ...
    // Popups
    "popupLayer", "popupCard", "popupBlackboardArt", "popupWhiteboardArt",
    "popupCloseButton",
    ...
    "confirmStopAccept", "confirmResetOverlay", "confirmResetLine",
    "confirmResetCancel", "confirmResetAccept",
    // GAME
    ...
    "reviewUnderButton", "reviewOverButton", "reviewCloseButton",
    "shiftNewerButton", "shiftOlderButton", "shiftPositionLine",
    "shiftMostRecentNote",
  ...
  /* Write an initials string. Every place initials are shown goes through
     here - the odometer drums, the sidewalk player board, and the Shift
     Report's provider line - which is what makes it the one place to teach
     if a symbol ever needs special treatment.

     It wraps the music-unlock symbol so CSS can colour that ONE character
     (John, 2026-08-08: it was unreadable in the drum). This is only
     possible because the symbol is a TEXT glyph - a colour emoji would
     carry its own palette and ignore the class, which is exactly why the
     wrapper was deleted for the day (2026-08-09) that six emoji named six
     playlists, and why it came back with ♫ on 2026-08-10. Every place
     initials are shown goes through here. */
  ...
  function renderHomeBoardSummaries(state) {
    ...
    /* Both review lines need at least one stored shift, so they dim
       together (TODO 11). Quitting a shift no longer darkens them: the
       lines offer the shifts already played, which a quit does not
       touch. */
    ui.reviewPastShiftsButton.disabled = !GAME.canReviewPastShifts(state);
    ui.resetPastShiftsButton.disabled = !GAME.canResetPastShifts(state);
  }

  /* What each loading step is called on screen. The splash and the home
     status line share this one vocabulary so a player never sees the same
     work described two different ways (2026-08-09). The per-patient
     counter that used to live here went with the 160 separate fetches:
     there is one compilation file now, so "37/160" would be a fiction. */
  const LOADING_STEP_TEXT = {
    art: "Loading artwork…",
    patients: "Loading patients…",
    artwork: "Loading artwork…"
  };

  function loadingStepText(loading) {
    return LOADING_STEP_TEXT[loading.step] || "Loading…";
  }
  ...
  function renderLoadingStatus(loading) {
    ...
    if (loading.failed) {
      ...
      statusElement.textContent = loadingStepText(loading);
    }
  ...
  /* The boot splash COVER is up from the first paint - it has no `hidden`
     in the markup, because a cover switched on by JavaScript arrives after
     the page has already painted (John, desktop, 2026-08-09). So there is
     nothing here to show: this only reveals the WORDS, which app.js holds
     back until loading is slow enough to deserve a loading screen. */
  function showBootSplashMessage(loading) {
    ui.bootSplash.classList.add("is-slow");
    ui.bootSplashStatus.textContent = loading.failed
      ? loading.errorMessage
      : loadingStepText(loading);
  }

  /* Lift the cover. Called once, when the entrance art is ready to paint. */
  ...
  function openPopup(popupName, state, triggerElement) {
    ...
    /* The board art is CHOSEN here, never LOADED here (TODO 18, fixed
       2026-08-09). Both images already carry their src and were decoded
       during boot, so opening a board costs one visibility toggle and the
       art cannot lag the lettering. This function must stay synchronous -
       ui.js is render-only, and awaiting anything here would race the
       focus() call at the end. */
    ui.popupBlackboardArt.hidden = isAbout;
    ui.popupWhiteboardArt.hidden = !isAbout;
  ...
  /* app.js registers one listener here and it survives every rebuild of
     the drums, so nothing has to be re-wired when the board reopens. */
  let playerWheelListener = null;

  function buildWheel(mountElement, values, startValue, pickerLabel, onChange) {
    const announceChange = () => { if (onChange) onChange(); };
  ...
  function buildPlayerWheels(state) {
    ...
    /* Always three drums, even if a stored name is shorter.
       ...
       The initials drums REPORT EVERY STEP (John, 2026-08-09), because the
       middle one chooses the playlist and the right one chooses the track,
       and he wants to hear that while spinning rather than after pressing
       the green check. The title drum is deliberately silent: a title
       cannot change what plays. */
    ui.playerInitialsWheels.replaceChildren();
    const initials = [];
    const announce = () => { if (playerWheelListener) playerWheelListener(); };
    for (let position = 0; position < constants.INITIALS_LENGTH; position += 1) {
      ...
      initials.push(buildWheel(mountElement, constants.INITIAL_SYMBOLS,
        symbols[position] || "A", `Initial ${position + 1}`, announce));
    }

    playerWheels = { title, initials };
  }

  /* What the PLAYER NAME drums currently read - the pending name, which is
     not what state says until the green check is pressed. The music
     audition needs it for the same reason the sound rows need
     pendingSoundSelections(). */
  function pendingPlayerName() {
    if (!playerWheels) return null;
    return {
      title: playerWheels.title.value(),
      initials: playerWheels.initials.map((wheel) => wheel.value()).join("")
    };
  }

  function setPlayerWheelListener(listener) {
    playerWheelListener = listener;
  }
  ...
  /* Names the number it is about to destroy, so "delete" is never a
     vague promise (TODO 11). The verb is DELETE throughout, matching the
     board's own wording (John, 2026-08-09). */
  function renderConfirmReset(state) {
    ui.confirmResetOverlay.hidden = state.overlay !== "confirm-reset";
    if (state.overlay !== "confirm-reset") return;
    const total = state.shiftHistory.length;
    const isSingle = total === 1;
    ui.confirmResetLine.textContent =
      `This deletes ${isSingle ? "your 1 stored shift" : `all ${total} stored shifts`}. It cannot be undone.`;
    ui.confirmResetAccept.textContent =
      isSingle ? "Yes, delete it" : "Yes, delete them all";
    ui.confirmResetCancel.focus();
  }
  ...
  function renderWaiting(state, portraitUrlFor) {
    ...
    /* The scene belongs to the ROW POSITION and holds still for the whole
       shift (2026-08-09), which is what stopped the panel blinking blank
       when a patient arrived: every render rebuilds these rows, and a new
       <img> given a url it has never held has nothing to paint for a frame.
       Same url every time, no blink - exactly why the rooms panel, which
       rebuilds just as hard, never flickered.

       The fallback covers HOME, where no shift has drawn a set yet and
       renderAll still runs. */
    const backgroundKeyFor = (rowIndex) =>
      state.waitingBackgrounds[rowIndex]
      || assets.waitingBackgroundKeys[
        rowIndex % assets.waitingBackgroundKeys.length];
    ...
    state.waiting.forEach((entry, waitingIndex) => {
      ...
      background.alt = "";
      background.src =
        assets.game.waitingBackgrounds[backgroundKeyFor(waitingIndex)];
    ...
    for (let i = state.waiting.length; i < rowCount; i++) {
      ...
      background.className = "queue-background";
      background.alt = "";
      background.src = assets.game.waitingBackgrounds[backgroundKeyFor(i)];
      const frame = document.createElement("span");
      frame.className = "queue-cell-frame";
  ...
  function renderChartOverlay(state, portraitUrlFor) {
    ...
    /* UNHIDE LAST, and that ordering IS the fix (2026-08-09). This used to
       run first, so the clipboard appeared and the chart was then built,
       scrolled and mounted into it - the player saw an empty board for a
       frame. The chart has no artwork of its own, so nothing here needs to
       wait on a download; it only needs to be assembled before it is
       shown. The portrait is the same image already painted in the patient
       panel, so it is decoded in-document and arrives with the rest. */
    ui.chartOverlay.hidden = false;
  }
  ...
  /* BUG FIX 2026-08-08: a spent numeral must not be left loaded in the
     element. The animation's `forwards` fill leaves it invisible, so the
     stale "1" from the end of a shift was easy to miss - until the GAME
     view was hidden and shown again for the NEXT shift, which restarts a
     CSS animation on a re-displayed element. The player saw a phantom "1"
     flash before their first patient (John, 2026-08-08).

     Two clears, because neither covers the other case: the animationend
     listener handles every ordinary numeral, but an animation running
     when the view is hidden is CANCELLED, not ended, so the last numeral
     of a shift never fires it. app.js therefore also clears at shift
     start, which is the moment that matters. */
  function clearCountdownNumeral() {
    ui.countdownNumeral.classList.remove("is-popping");
    ui.countdownNumeral.textContent = "";
  }

  ui.countdownNumeral.addEventListener("animationend", clearCountdownNumeral);
  ...
  function renderReview(state) {
    ...
    /* The browse arrows and their counter exist ONLY while a stored
       shift is on screen (TODO 11) - never on the report of the shift
       just finished, whose newest-stored twin is itself. One state
       field decides all three elements. */
    const position = GAME.selectStoredShiftPosition(state);
    const isBrowsing = position !== null;
    ui.shiftNewerButton.hidden = !isBrowsing;
    ui.shiftOlderButton.hidden = !isBrowsing;
    ui.shiftPositionLine.hidden = !isBrowsing;
    /* Not browsing: the note is display:none too, so its grid row
       collapses and an unbrowsed report lays out exactly as it always
       did. Browsing: the row is always present and only its VISIBILITY
       changes, so stepping between #1 and #2 moves nothing. */
    ui.shiftMostRecentNote.hidden = !isBrowsing;
    if (isBrowsing) {
      ui.shiftPositionLine.textContent =
        `${position.number} OF ${position.total}`;
      ui.shiftMostRecentNote.classList.toggle("is-hidden", position.number !== 1);
    }
